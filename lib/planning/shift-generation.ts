...
export function generateFlexiblePoolShifts(
  ...
  // Cumulative counted hours already committed to each employee THIS
  // WEEK, from every day processed before this one (both generated-day
  // picks and any static/baseline day) -- see generate-draft-plan.ts,
  // which threads this forward day by day. Combined with
  // maximumWeeklyWorkingHours, this is the generation-time half of the
  // confirmed 42h ceiling: a candidate shift that would push an
  // employee's running weekly total past the ceiling is hard-excluded
  // here, at SELECTION time, never generated and left for
  // validation.ts's checkWeeklyHoursCeiling to merely flag afterward.
  hoursSoFarThisWeek: Map<string, number> = new Map(),
  maximumWeeklyWorkingHours = Infinity,
  ...
  // Each employee's own STATIC baseline hours for every day STRICTLY
  // AFTER today that this run hasn't reached yet (see
  // generate-draft-plan.ts's futureBaselineHours) -- a conservative,
  // worst-case assumption that none of those future days get overridden
  // by Stage 6 either. Without this, today's candidate shift is only
  // checked against hours ALREADY committed so far this week, never
  // against hours the employee is already fixed to accrue later in the
  // SAME week from days generation hasn't even reached yet -- a real,
  // observed gap once the confirmed ceiling could be crossed by mixing
  // Stage-6-chosen days with untouched baseline days (see the delivered
  // report). Same "conservative, sometimes overcautious" tradeoff as
  // nextDayBaselineShift: if a later day turns out to be re-generated
  // with a shorter code, today's rejection here was stricter than
  // strictly necessary, but never generates an illegal roster.
  futureBaselineHours: Map<string, number> = new Map()
  ...
  for (const role of rolesToConsider) {
    ...
    const shiftDuration = getShiftDurationHours(shiftCode);
    ...
    for (const employee of availableToday) {
      ...
      // Hard 42h-ceiling gate: never hand out a shift that would push this
      // employee's running weekly total past the confirmed
      // maximumWeeklyWorkingHours, even though it's otherwise the correct
      // coverage shift for today. This is a genuine capacity shortfall for
      // today's demand, not something scoring/validation should "rescue"
      // or merely flag after the fact.
      const committedSoFar = hoursSoFarThisWeek.get(employee.id) ?? 0;
      const stillToComeFromBaseline = futureBaselineHours.get(employee.id) ?? 0;
      if (committedSoFar + shiftDuration + stillToComeFromBaseline > maximumWeeklyWorkingHours) {
        continue;
      }
