...
export function RequirementDetail({
  ...
  const { requirement, assignedEmployees, proposedEmployees, coverageStatus, coverageLabel } = view;

  return (
    <div className="flex flex-col gap-2 rounded-lg bg-surface px-3 py-2.5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="text-sm">
          <span className="font-medium text-ink">{coverageLabel}</span>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {/* ATLAS's own draft-plan assignments -- a normal part of the
            generated plan, not a pending recommendation, so this is styled
            the same calm brand blue used everywhere else for
            "generated by ATLAS", never a "needs approval" treatment. */}
 ...
 * Uses `coverageLabel` (Gate/Boarding/Profiling/Mesure for RAM, or
 * "{Airline} Team" for a foreign-carrier requirement) -- never the raw
 * internal skill tag "Ramp Team" surfaced as if it were an airport
 * ramp-handling function. A needs_configuration requirement never reaches
 * this component at all -- buildRosterViews excludes it from `roster`
 * entirely (see weekly-plan-view.ts), so there is nothing routine to
 * render for it here.
 */
function RequirementChip({ view }: { view: RosterRequirementView }) {
  const { assignedEmployees, proposedEmployees, coverageStatus, coverageLabel, requirement } = view;
  ...
  return (
    <Badge tone={tone}>
      {coverageLabel} {covered}/{requirement.total_requirement}
    </Badge>
  );
}
